...
abstract class GraphTraversable[T] {
  ...
  def findPath(seed: ListBuffer[Path]): Path = {
    ...
    def traverseGraph(candidatePaths: ListBuffer[Path], exploredVertices: Set[T]): Path = {
      candidatePaths.headOption match {
        case None => Nil
        case Some(pathToCurrentVertex) =>
          if (isSolution (pathToCurrentVertex) ) pathToCurrentVertex
          else {
            val adjacentVertices = adjVertices (pathToCurrentVertex.head).filterNot(exploredVertices.contains(_)).filter (isVertexEligibleForPath (_, pathToCurrentVertex) )
