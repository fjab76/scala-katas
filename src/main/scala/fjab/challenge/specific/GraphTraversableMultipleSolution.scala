 ...
 *
 */
abstract class GraphTraversableMultipleSolution[T] {
  ...
  /**
    ...
    */
  def findPath(seed: ListBuffer[Path]): List[Path] = {

    val solution: ListBuffer[Path] = new ListBuffer()
    var count = 0
    def traverseGraph(candidatePaths: ListBuffer[Path], exploredVertices: Set[T]): List[Path] = {
      candidatePaths.headOption match {
        case None => solution.toList.map(_.reverse).sortWith(_.length > _.length)
        case Some(pathToCurrentVertex) =>
          if (isSolution (pathToCurrentVertex) ) {
            count += 1
            solution += pathToCurrentVertex
            candidatePaths.remove(0)
            if(candidatePaths.isEmpty || count == 20){
              solution.toList.map(_.reverse).sortWith(_.length > _.length)
            }
            else{
              traverseGraph(candidatePaths, exploredVertices + pathToCurrentVertex.head)
            }
          }
          else {
