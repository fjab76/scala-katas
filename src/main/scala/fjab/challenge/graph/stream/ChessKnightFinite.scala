...
class ChessKnightFinite(to: Coordinate)(x: Int, y: Int) extends GraphTraverser[Coordinate]{
  ...
  def findShortestPathFrom(from: Coordinate): Stream[Path] = findPath(List(List(from)))
   ...
   */
  override def addPathsToNeighbours(listOfPaths: Stream[Path], pathsToAdjacentVertices: List[Path]): Stream[Path] =
